...
export class EventHandlerManager implements AppModule {
  ...
  startHeaderClock(): void {
    ...
    const istEl = document.getElementById('headerClockIST');
    if (!el && !istEl) return;
    const tick = () => {
      const now = new Date();
      if (el) {
      ...
      if (istEl) {
        ...
        // Manually construct format to match toUTCString style: "Wkd, DD Mon YYYY HH:MM:SS"
        ...
        istEl.textContent = `${getPart('weekday')}, ${getPart('day')} ${getPart('month')} ${getPart('year')} ${getPart('hour')}:${getPart('minute')}:${getPart('second')} IST`;
      }
    };
    tick();

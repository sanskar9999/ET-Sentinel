...
export class PanelLayoutManager implements AppModule {
    ...
    this.ctx.container.innerHTML = `
        ...
        return `
          ...
          <div class="region-selector">
            <select id="regionSelect" class="region-select">
               ...
               <option value="india" selected>${t('components.deckgl.views.india')}</option>
      ...
      <div class="region-bottom-sheet" id="regionBottomSheet">
        ...
        ${[
         ...
         { value: 'india', label: t('components.deckgl.views.india') },
        ...
        { value: 'oceania', label: t('components.deckgl.views.oceania') },
         ...
         `<button class="region-sheet-option ${r.value === 'india' ? 'active' : ''}" data-region="${r.value}">
           <span>${r.label}</span>
           <span class="region-sheet-check">${r.value === 'india' ? '✓' : ''}</span>
        </button>`
      ).join('')}
      </div>
        ...
        <div class="map-section" id="mapSection">
          <div class="panel-header">
            ...
            <div class="header-clock-wrap">
              <span class="header-clock" id="headerClockIST" translate="no"></span>
  ...
  private createPanels(): void {
    ...
    const preferGlobe = SITE_VARIANT === 'et' || loadFromStorage<string>(STORAGE_KEYS.mapMode, 'flat') === 'globe';
    this.ctx.map = new MapContainer(mapContainer, {
      zoom: this.ctx.isMobile ? 2.5 : 1.0,
      pan: { x: 0, y: 0 },
      view: this.ctx.resolvedLocation,
    ...
    // ET Sentinel: center map on India by default
    if (SITE_VARIANT === 'et') {
      this.ctx.map.setCenter(20.5, 78.9, 2);
    }
    ...
    // ET Sentinel — Story Arc Causal Intelligence panel
    if (SITE_VARIANT === 'et') {
      import('@/components/StoryArcPanel').then(({ StoryArcPanel }) => {
        const storyArcPanel = new StoryArcPanel();
        this.ctx.panels['story-arc'] = storyArcPanel;
        const el = storyArcPanel.getElement();
        this.makeDraggable(el, 'story-arc');
        const grid = document.getElementById('panelsGrid');
        if (grid) {
          el.classList.add('panel-wide');
          // Insert at the very top of the grid (index 0)
          if (grid.firstChild) {
  ...
  private makeDraggable(el: HTMLElement, key: string): void {
    // Disable global panel dragging as requested to prioritize inner-panel interactions (like graph panning).
    return;

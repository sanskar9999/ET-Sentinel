...
// ET variant: Indian business news
const ET_LIVE_CHANNELS: LiveChannel[] = [
  { id: 'et-now', name: 'ET Now', handle: '@etnow', fallbackVideoId: 'y-mYvG_t8sM' },
  { id: 'cnbc-tv18', name: 'CNBC TV18', handle: '@cnbctv18', fallbackVideoId: 'f0l8wYyq_uA' },
  { id: 'ndtv-profit', name: 'NDTV Profit', handle: '@ndtvprofit', fallbackVideoId: 'pS-W9ZlY-Bw' },
  { id: 'wion', name: 'WION', handle: '@WION', fallbackVideoId: 'h8L7fN_W6z8' },
  { id: 'india-today', name: 'India Today', handle: '@indiatoday', fallbackVideoId: 'sYZtOFzM78M' },
  { id: 'bloomberg', name: 'Bloomberg', handle: '@markets', fallbackVideoId: 'iEpJwprxDdk' },
];

const DEFAULT_LIVE_CHANNELS = SITE_VARIANT === 'tech' ? TECH_LIVE_CHANNELS : SITE_VARIANT === 'et' ? ET_LIVE_CHANNELS : SITE_VARIANT === 'happy' ? [] : FULL_LIVE_CHANNELS;
...
export class LiveNewsPanel extends Panel {
  ...
  private isMuted = SITE_VARIANT !== 'et';
  ...
  constructor() {
    ...
    if (this.channels.length === 0 || SITE_VARIANT === 'et') this.channels = getDefaultLiveChannels();
    const savedChannelId = SITE_VARIANT === 'et' ? '' : loadFromStorage<string>(STORAGE_KEYS.activeChannel, '');
    ...
    if (SITE_VARIANT === 'et') {
      setTimeout(() => this.triggerInit(), 500);
    }
  }

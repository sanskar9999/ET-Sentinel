...
export class GlobeMap {
  ...
  private async initGlobe(): Promise<void> {
    ...
    controls.autoRotate = SITE_VARIANT === 'et' || !desktop;
    controls.autoRotateSpeed = 0.3;
    controls.enablePan = SITE_VARIANT !== 'et';
    ...
    // Update zoom level indicator on camera change
    controls.addEventListener('change', () => {
      if (this.globe && this.zoomIndicatorEl) {
        const pov = this.globe.pointOfView();
        this.zoomIndicatorEl.innerHTML = `<span style="opacity: 0.6;">Altitude</span> <span>${pov.altitude.toFixed(2)}</span>`;
      }
    });
    ...
    zoomIndicator.className = 'globe-zoom-indicator';
    ...
    this.zoomIndicatorEl = zoomIndicator;

    // Initial zoom value
    const initialPov = globe.pointOfView();
    if (this.zoomIndicatorEl) {
      this.zoomIndicatorEl.innerHTML = `<span style="opacity: 0.6;">Altitude</span> <span>${initialPov.altitude.toFixed(2)}</span>`;
    }
  ...
  private static readonly VIEW_POVS: Record<MapView, { lat: number; lng: number; altitude: number }> = {
    global:   { lat: 20,  lng:  0,   altitude: 1.5 },
    ...
    india:    { lat: 20.5, lng: 78.9,  altitude: 1.5 },
  };
